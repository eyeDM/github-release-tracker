...
Новый формат `REPOS` поддерживает два варианта записи:

- Простая строка, как раньше: `"owner/repo"` (обратно совместимо).
- Объект с дополнительной опцией `asset_regex` для выбора файла в секции `assets` релиза по регулярному выражению:
...
Если для релиза найден asset, имя и прямая ссылка на `browser_download_url` будут добавлены в уведомление в Telegram (HTML-ссылка). Выбирается первый asset, имя которого соответствует регулярному выражению. Неверные регулярные выражения игнорируются, запись обрабатывается без выбора asset.
